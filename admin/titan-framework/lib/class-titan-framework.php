		...
		if ( ! empty( $option->settings['id'] ) ) {
			...
			if ( is_admin() && isset( $this->optionsUsed[ $option->settings['id'] ] ) ) {
				self::displayFrameworkError(
					sprintf( __( 'All option IDs per namespace must be unique. The id %s has been used multiple times.', 'gpress' ),
   ...
   *
	...
	public function createContainer( $settings ) {
		if ( empty( $settings['type'] ) ) {
			self::displayFrameworkError( sprintf( __( '%s needs a %s parameter.', 'gpress' ), '<code>' . __FUNCTION__ . '</code>', '<code>type</code>' ) );
			return;
		}
		...
		if ( ! class_exists( $class ) ) {
			self::displayFrameworkError( sprintf( __( 'Container of type %s, does not exist.', 'gpress' ), '<code>' . $settings['type'] . '</code>' ) );
			return;
		}
